...
def substitute_synonyms(sentence, synonyms):
  ...
  result = _substitute_synonyms(s_array, synonyms)
...
def _substitute_synonyms(sentence, synonyms):
  ...
  if word in synonyms:
    for synonym in synonyms[word]:
      for partial in _substitute_synonyms(sentence[1:], synonyms):
        result.append([synonym, *partial])
  else:
    for substring in _substitute_synonyms(sentence[1:], synonyms):
