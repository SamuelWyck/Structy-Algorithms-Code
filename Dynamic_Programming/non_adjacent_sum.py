...
def _non_adjacent_sum(nums, memo, pointer):
  if pointer >= len(nums):
    return 0
  if pointer in memo:
    return memo[pointer]

  take = nums[pointer] + _non_adjacent_sum(nums, memo, pointer + 2)
  dont_take = _non_adjacent_sum(nums, memo, pointer + 1)

  
  memo[pointer] = max(take, dont_take)
  return memo[pointer]
